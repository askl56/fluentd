...
class HttpInputTest < Test::Unit::TestCase
  ...
  def test_json
    ...
    d.emit_streams.each do |_tag, es|
  ...
  def test_application_json
    ...
    d.run do
      d.expected_emits.each do|tag, time, record|
        res = post("/#{tag}?time=#{time}", record.to_json, 'content-type' => 'application/json; charset=utf-8')
        assert_equal '200', res.code
      end
    end
  end
  ...
  def test_with_regexp
    ...
    d.run do
      d.expected_emits.each do |tag, time, record|
        body = record.map do |_k, v|
          v.to_s
        end.join(':')
        res = post("/#{tag}?time=#{time}", body)
        assert_equal '200', res.code
      end
    end
  end
