    ...
    %w(yes no).each do |enable_ruby|
      ...
      test "disabled typecasting of values with enable_ruby #{enable_ruby}" do
        ...
        expected_results = [
          ...
          { single: 123.to_s,
            multiple: "#{123}#{123}",
            with_prefix: "prefix-#{123}",
            with_suffix: "#{123}-suffix" },
          { single: [1, 2].to_s,
            multiple: "#{[1, 2]}#{[1, 2]}",
            with_prefix: "prefix-#{[1, 2]}",
            with_suffix: "#{[1, 2]}-suffix" },
          { single: { a: 1, b: 2 }.to_s,
            multiple: "#{{ a: 1, b: 2 }}#{{ a: 1, b: 2 }}",
            with_prefix: "prefix-#{{ a: 1, b: 2 }}",
            with_suffix: "#{{ a: 1, b: 2 }}-suffix" },
          { single: nil.to_s,
            multiple: "#{nil}#{nil}",
            with_prefix: "prefix-#{nil}",
            with_suffix: "#{nil}-suffix" }
        ]
        actual_results = []
        es = emit(config, msgs)
        es.each_with_index do |(_t, r), _i|
