...
class ForwardInputTest < Test::Unit::TestCase
  ...
  def test_forward
    ...
    d.run do
      entries = []
      d.expected_emits.each do|_tag, time, record|
        entries << [time, record]
      end
  ...
  def test_send_large_chunk_warning
    ...
    # check log
    assert d.instance.log.logs.count{|line|
    ...
    } == 1, 'large chunk warning is not logged'
  end
  ...
  def test_send_broken_chunk(data)
    ...
    # d.run => send_data
    d.run do
      d.instance.send(:on_message, data, 1_000_000_000, 'host: 127.0.0.1, addr: 127.0.0.1, port: 0000')
    end
    ...
    } == 1, 'should not accept broken chunk'
  end
  ...
  def test_respond_to_forward_requiring_ack
    ...
    d.run do
      entries = []
      events.each do|_tag, time, record|
        entries << [time, record]
      end
  ...
  def send_data(data, try_to_receive_response = false, response_timeout = 1)
    ...
    begin
      ...
      if try_to_receive_response
        res = io.recv(1024) if IO.select([io], nil, nil, response_timeout)
