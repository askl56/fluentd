    ...
    sub_test_case 'class defined with config_section' do
      ...
      sub_test_case '#configure' do
        ...
        test 'checks missing of specifications' do
          ...
          complete = e('ROOT', '',             'stringvalue' => 's1', 'boolvalue' => 'yes', 'integervalue' => '10',
                                               'sizevalue' => '10m', 'timevalue' => '100s', 'floatvalue' => '1.001',
                                               'hashvalue' => '{"foo":1, "bar":2}',
                                               'arrayvalue' => '[1,"ichi"]')

          checker = ->(conf) { ConfigurableSpec::Example0.new.configure(conf) }

          assert_nothing_raised { checker.call(complete) }
          assert_raise(Fluent::ConfigError) { checker.call(complete.reject { |k, _v| k == 'stringvalue' }) }
          assert_raise(Fluent::ConfigError) { checker.call(complete.reject { |k, _v| k == 'boolvalue'   }) }
          assert_raise(Fluent::ConfigError) { checker.call(complete.reject { |k, _v| k == 'integervalue' }) }
          assert_raise(Fluent::ConfigError) { checker.call(complete.reject { |k, _v| k == 'sizevalue'   }) }
          assert_raise(Fluent::ConfigError) { checker.call(complete.reject { |k, _v| k == 'timevalue'   }) }
          assert_raise(Fluent::ConfigError) { checker.call(complete.reject { |k, _v| k == 'floatvalue'  }) }
          assert_raise(Fluent::ConfigError) { checker.call(complete.reject { |k, _v| k == 'hashvalue'   }) }
          assert_raise(Fluent::ConfigError) { checker.call(complete.reject { |k, _v| k == 'arrayvalue'  }) }
        end

        test 'accepts configuration values as string representation' do
          conf = e('ROOT', '',             'stringvalue' => 's1', 'boolvalue' => 'yes', 'integervalue' => '10',
        ...
        test 'accepts configuration values as ruby value representation (especially for DSL)' do
          conf = e('ROOT', '',             'stringvalue' => 's1', 'boolvalue' => true, 'integervalue' => 10,
        ...
        test 'gets both of true(yes) and false(no) for bool value parameter' do
          conf = e('ROOT', '',             'stringvalue' => 's1', 'integervalue' => 10,
                                           'sizevalue' => 10 * 1024 * 1024, 'timevalue' => 10 * 60, 'floatvalue' => 1.001,
                                           'hashvalue' => { 'foo' => 1, 'bar' => 2 },
                                           'arrayvalue' => [1, 'ichi'])
